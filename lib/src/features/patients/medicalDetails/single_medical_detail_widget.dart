import 'package:easy_localization/easy_localization.dart';
import 'package:flutter/material.dart';
import 'package:flutter/scheduler.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';
import 'package:health_care/constants/global_variables.dart';
import 'package:health_care/models/vital_signs.dart';
import 'package:health_care/providers/auth_provider.dart';
import 'package:health_care/providers/data_grid_provider.dart';
import 'package:health_care/providers/vital_provider.dart';
import 'package:health_care/services/auth_service.dart';
import 'package:health_care/services/time_schedule_service.dart';
import 'package:health_care/services/vital_service.dart';
import 'package:health_care/shared/custom_pagination_widget.dart';
import 'package:health_care/shared/gradient_button.dart';
import 'package:health_care/shared/sf_data_grid_filter_widget.dart';
import 'package:health_care/shared/vital_image.dart';
import 'package:health_care/src/commons/fadein_widget.dart';
import 'package:health_care/src/commons/scaffold_wrapper.dart';
import 'package:health_care/src/commons/scroll_button.dart';
import 'package:health_care/src/features/patients/medicalDetails/add_medical_detail_form.dart';
import 'package:health_care/src/features/patients/medicalDetails/patient_medical_detail_show_card.dart';
import 'package:health_care/stream_socket.dart';
import 'package:provider/provider.dart';
import 'package:syncfusion_flutter_datagrid/datagrid.dart';

class SingleMedicalDetailWidget extends StatefulWidget {
  static const String routeName = '/patient/dashboard/singleMedicalDetail';
  final String title;
  const SingleMedicalDetailWidget({
    super.key,
    required this.title,
  });

  @override
  State<SingleMedicalDetailWidget> createState() => _SingleMedicalDetailWidgetState();
}

class _SingleMedicalDetailWidgetState extends State<SingleMedicalDetailWidget> {
  final ScrollController scrollController = ScrollController();
  final AuthService authService = AuthService();
  late final AuthProvider authProvider;
  final VitalService vitalService = VitalService();
  late final VitalProvider vitalProvider;
  late final DataGridProvider dataGridProvider;
  late List<int> deleteIds;
  bool _isProvidersInitialized = false;
  double scrollPercentage = 0;

  Future<void> getDataOnUpdate() async {
    vitalService.getVitalSignForMedicalRecords(context, widget.title);
    setState(() {
      deleteIds.clear();
    });
  }

  @override
  void initState() {
    super.initState();
    authService.updateLiveAuth(context);
    deleteIds = [];
    WidgetsBinding.instance.addPostFrameCallback((_) {
      getDataOnUpdate();
    });
  }

  Future<void> getConfirmationForDeleteMedicalDetail(
    BuildContext context,
    List<int> deleteIds,
  ) async {
    final result = await showModalBottomSheet<List<int>>(
      context: context,
      isScrollControlled: true,
      builder: (context) => FractionallySizedBox(
        heightFactor: 0.4,
        child: Scaffold(
          appBar: AppBar(
            backgroundColor: Theme.of(context).primaryColorLight,
            title: Text("deleteMedicalRecordsRequest_${widget.title}").plural(
              deleteIds.length,
              format: NumberFormat.compact(
                locale: context.locale.toString(),
              ),
            ),
            automaticallyImplyLeading: false, // Removes the back button
            actions: [
              IconButton(
                icon: Icon(
                  Icons.close,
                  color: Theme.of(context).primaryColor,
                ),
                onPressed: () => Navigator.pop(context),
              ),
            ],
          ),
          body: Padding(
            padding: const EdgeInsets.all(16.0),
            child: Column(
              children: [
                Padding(
                  padding: const EdgeInsets.all(8),
                  child: Card(
                    elevation: 6,
                    color: Theme.of(context).canvasColor,
                    shape: RoundedRectangleBorder(
                      side: BorderSide(color: Theme.of(context).primaryColorLight),
                      borderRadius: const BorderRadius.all(Radius.circular(15)),
                    ),
                    child: Padding(
                      padding: const EdgeInsets.all(16.0),
                      child: Center(
                          child: Text(
                        context.tr("confirmDeleteMedicalRecords"),
                        style: TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.normal,
                          color: Theme.of(context).primaryColor,
                        ),
                      )),
                    ),
                  ),
                ),
                const SizedBox(height: 20),
                SizedBox(
                  width: double.infinity,
                  child: ElevatedButton(
                    onPressed: () {
                      Navigator.pop(context, deleteIds);
                    },
                    child: Text(context.tr("submit")),
                  ),
                )
              ],
            ),
          ),
        ),
      ),
    );
    if (!context.mounted) return;

    if (result != null) {
      deleteMedicalRecordRequestSubmit(context, result);
    }
  }

  Future<void> deleteMedicalRecordRequestSubmit(BuildContext context, List<int> deleteIds) async {
    bool success = await vitalService.vitalSignDelete(context, widget.title, deleteIds);

    if (success) {
      if (context.mounted) {
        showErrorSnackBar(context, "${deleteIds.length} Records(s) was deleted.");
      }
      setState(() {
        deleteIds.clear();
      });
      getDataOnUpdate();
    }
  }

  Future<void> openAddMedicalRecor(
    BuildContext context,
  ) async {
    final matchedVital = vitalBoxList.firstWhere(
      (element) => element['title'] == widget.title,
      orElse: () => {},
    );
    var brightness = Theme.of(context).brightness;
    final String image = matchedVital['image'] ?? '';
    final String icon = matchedVital['icon'] ?? '';
    final result = await showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      useSafeArea: true,
      builder: (context) => FractionallySizedBox(
        heightFactor: 1.0,
        child: SafeArea(
          child: Scaffold(
            appBar: AppBar(
              backgroundColor: Theme.of(context).primaryColor,
              title: Row(
                mainAxisAlignment: MainAxisAlignment.start,
                children: [
                  SizedBox(
                    height: 30,
                    width: 30,
                    child: VitalImage(
                      image: image,
                      icon: icon,
                      brightness: brightness,
                    ),
                  ),
                  const SizedBox(width: 10),
                  Text(context.tr("add_${widget.title}")),
                ],
              ),
              automaticallyImplyLeading: false, // Removes the back button
              actions: [
                IconButton(
                  icon: Icon(
                    Icons.close,
                    color: Theme.of(context).primaryColorLight,
                  ),
                  onPressed: () => Navigator.pop(context),
                ),
              ],
            ),
            body: AddMedicalDetailForm(title: widget.title),
          ),
        ),
      ),
    );

    if (!context.mounted) return;

    if (result != null) {
      final intValue = int.tryParse(result["value"]);
      String userId = authProvider.patientProfile!.userId;
      var payload = {
        "name": widget.title,
        "value": intValue,
        "userId": userId,
      };
      socket.emit('vitalSignsUpdate', payload);
      getDataOnUpdate();
    }
  }

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (!_isProvidersInitialized) {
      authProvider = Provider.of<AuthProvider>(context, listen: false);
      vitalProvider = Provider.of<VitalProvider>(context, listen: false);
      dataGridProvider = Provider.of<DataGridProvider>(context, listen: false);
      _isProvidersInitialized = true;
    }
  }

  @override
  void dispose() {
    scrollController.dispose();
    dataGridProvider.setMongoFilterModel({}, notify: false);
    socket.off('getVitalSignForMedicalRecordsReturn');
    socket.off('updateVitalSignForMedicalRecords');
    socket.off('vitalSignDeleteReturn');
    socket.off('vitalSignsUpdate');
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    return Consumer<VitalProvider>(
      builder: (context, vitalProvider, child) {
        final int totalRecords = vitalProvider.total;
        final List<VitalSignValues> vitalSignValues = vitalProvider.vitalSignValues;
        final bool isLoading = vitalProvider.isLoading;
        final String title = widget.title;
        final ThemeData theme = Theme.of(context);
        final Color textColor = theme.brightness == Brightness.dark ? Colors.white : Colors.black;

        var brightness = theme.brightness;
        // Filter the list to find the item with matching title
        final matchedVital = vitalBoxList.firstWhere(
          (element) => element['title'] == title,
          orElse: () => {},
        );
        bool isFilterActive = dataGridProvider.mongoFilterModel.isNotEmpty;
        final List<FilterableGridColumn> filterableColumns = [
          FilterableGridColumn(column: GridColumn(columnName: 'id', label: Text(context.tr('id'))), dataType: 'number'),
          FilterableGridColumn(column: GridColumn(columnName: 'value', label: Text(context.tr('value'))), dataType: 'number'),
          FilterableGridColumn(column: GridColumn(columnName: 'date', label: Text(context.tr('date'))), dataType: 'date'),
        ];

        // Extract image and icon safely with fallback empty string
        final String image = matchedVital['image'] ?? '';
        final String icon = matchedVital['icon'] ?? '';
        return ScaffoldWrapper(
          title: context.tr(title),
          children: Stack(
            children: [
              NotificationListener<ScrollNotification>(
                onNotification: (notification) {
                  double per = 0;
                  if (scrollController.hasClients) {
                    per = ((scrollController.offset / scrollController.position.maxScrollExtent));
                  }
                  if (per >= 0) {
                    SchedulerBinding.instance.addPostFrameCallback((_) {
                      if (context.mounted) {
                        setState(() {
                          scrollPercentage = 307 * per;
                        });
                      }
                    });
                  }
                  return false;
                },
                child: SingleChildScrollView(
                  controller: scrollController,
                  child: Column(
                    children: [
                      Padding(
                        padding: const EdgeInsets.only(top: 8.0),
                        child: SizedBox(
                          height: 35,
                          child: Padding(
                            padding: const EdgeInsets.symmetric(horizontal: 16.0),
                            child: GradientButton(
                              onPressed: () {
                                openAddMedicalRecor(context);
                              },
                              colors: [
                                theme.primaryColorLight,
                                theme.primaryColor,
                              ],
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
                                  FaIcon(FontAwesomeIcons.circlePlus, size: 13, color: textColor),
                                  const SizedBox(width: 5),
                                  Text(
                                    context.tr("addMedicalDetail_$title"),
                                    style: TextStyle(fontSize: 12, color: textColor),
                                  )
                                ],
                              ),
                            ),
                          ),
                        ),
                      ),
                      FadeinWidget(
                        isCenter: true,
                        child: TotalMedicalDetailsBox(theme: theme, image: image, icon: icon, brightness: brightness, title: title, totalRecords: totalRecords),
                      ),
                      // confirmed delete
                      AnimatedSwitcher(
                        duration: const Duration(milliseconds: 300),
                        transitionBuilder: (Widget child, Animation<double> animation) {
                          return SizeTransition(
                            sizeFactor: animation,
                            axis: Axis.vertical,
                            child: child,
                          );
                        },
                        child: deleteIds.isNotEmpty
                            ? Padding(
                                key: const ValueKey("buttonVisible"),
                                padding: const EdgeInsets.symmetric(horizontal: 16.0),
                                child: SizedBox(
                                  width: MediaQuery.of(context).size.width,
                                  height: 40,
                                  child: GradientButton(
                                    onPressed: () async {
                                      getConfirmationForDeleteMedicalDetail(context, deleteIds);
                                    },
                                    colors: const [
                                      Colors.pink,
                                      Colors.red,
                                    ],
                                    child: Center(
                                      child: Text(
                                        "deleteMedicalRecordsRequest_$title",
                                        style: TextStyle(fontSize: 12, color: textColor),
                                      ).plural(
                                        deleteIds.length,
                                        format: NumberFormat.compact(
                                          locale: context.locale.toString(),
                                        ),
                                      ),
                                    ),
                                  ),
                                ),
                              )
                            : const SizedBox.shrink(key: ValueKey("buttonHidden")),
                      ),
                      const SizedBox(height: 10),
                      CustomPaginationWidget(
                        count: totalRecords,
                        getDataOnUpdate: getDataOnUpdate,
                      ),
                      const SizedBox(height: 10),
                      Row(
                        children: [
                          Checkbox(
                            splashRadius: 0,
                            checkColor: theme.primaryColorLight,
                            visualDensity: const VisualDensity(horizontal: 0, vertical: -4),
                            value: deleteIds.isNotEmpty,
                            onChanged: (bool? value) {
                              if (value == false) {
                                setState(() {
                                  deleteIds.clear();
                                });
                              } else {
                                setState(() {
                                  deleteIds = vitalSignValues.map((rev) => rev.id).toList();
                                });
                              }
                            },
                          ),
                          context.locale.toString() == 'th_TH'
                              ? SizedBox(
                                  width: MediaQuery.of(context).size.width - 90,
                                  child: Text(
                                    'taggleMedicalRecordForDelete_$title'.plural(
                                      vitalSignValues.length,
                                      format: NumberFormat.compact(locale: context.locale.toString()),
                                    ),
                                    maxLines: 2,
                                    softWrap: true,
                                    style: TextStyle(
                                      fontSize: 15.0,
                                      fontWeight: FontWeight.bold,
                                      color: theme.primaryColorLight,
                                    ),
                                  ),
                                )
                              : Text(
                                  'taggleMedicalRecordForDelete_$title',
                                  style: TextStyle(
                                    fontSize: 18,
                                    color: theme.primaryColorLight,
                                  ),
                                ).plural(
                                  vitalSignValues.length,
                                  format: NumberFormat.compact(
                                    locale: context.locale.toString(),
                                  ),
                                ),
                        ],
                      ),
                      ListView.builder(
                        shrinkWrap: true,
                        restorationId: 'vitalSign',
                        key: const ValueKey('vitalSign'),
                        physics: const NeverScrollableScrollPhysics(), // Disable ListView's own scrolling
                        itemCount: vitalSignValues.length,
                        itemBuilder: (context, index) {
                          final vitalSignValue = vitalSignValues[index];
                          return FadeinWidget(
                            isCenter: true,
                            child: PatientMedicalDetailShowCard(
                              title: title,
                              vitalSignValue: vitalSignValue,
                              getDataOnUpdate: getDataOnUpdate,
                              getConfirmationForDeleteMedicalDetail: getConfirmationForDeleteMedicalDetail,
                            ),
                          );
                        },
                      ),
                      if (isLoading) ...[
                        const Center(
                          child: Padding(
                            padding: EdgeInsets.all(16.0),
                            child: CircularProgressIndicator(),
                          ),
                        )
                      ],
                    ],
                  ),
                ),
              ),
              ScrollButton(scrollController: scrollController, scrollPercentage: scrollPercentage),
              Positioned(
                bottom: 10,
                left: 10,
                child: SizedBox(
                  width: 40,
                  height: 40,
                  child: FloatingActionButton(
                    tooltip: context.tr('filter'),
                    mini: true,
                    onPressed: () async {
                      final result = await showModalBottomSheet<Map<String, dynamic>>(
                        context: context,
                        isScrollControlled: true,
                        builder: (context) => FractionallySizedBox(
                          heightFactor: 0.9,
                          child: SfDataGridFilterWidget(columns: filterableColumns, columnName: 'id'),
                        ),
                      );

                      if (result != null) {
                        dataGridProvider.setPaginationModel(0, 10);
                        dataGridProvider.setMongoFilterModel({...result});
                        await getDataOnUpdate();
                      }
                    },
                    child: Stack(
                      clipBehavior: Clip.none,
                      children: [
                        Icon(
                          Icons.filter_alt_outlined,
                          size: 25,
                          color: theme.primaryColor,
                        ),
                        if (isFilterActive)
                          Positioned(
                            right: 0,
                            top: 0,
                            child: Container(
                              width: 6,
                              height: 6,
                              decoration: BoxDecoration(
                                color: theme.primaryColor,
                                shape: BoxShape.circle,
                              ),
                            ),
                          )
                      ],
                    ),
                  ),
                ),
              ),
            ],
          ),
        );
      },
    );
  }
}

class TotalMedicalDetailsBox extends StatelessWidget {
  const TotalMedicalDetailsBox({
    super.key,
    required this.theme,
    required this.image,
    required this.icon,
    required this.brightness,
    required this.title,
    required this.totalRecords,
  });

  final ThemeData theme;
  final String image;
  final String icon;
  final Brightness brightness;
  final String title;
  final int totalRecords;

  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.all(8.0),
      child: Card(
        elevation: 6,
        shape: RoundedRectangleBorder(
          side: BorderSide(color: theme.primaryColorLight),
          borderRadius: const BorderRadius.all(
            Radius.circular(15),
          ),
        ),
        child: Padding(
          padding: const EdgeInsets.all(16.0),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            SizedBox(
              height: 50,
              width: 50,
              child: VitalImage(
                image: image,
                icon: icon,
                brightness: brightness,
              ),
            ),
            context.locale.toString() == 'th_TH'
                ? SizedBox(
                    width: MediaQuery.of(context).size.width - 130,
                    child: Text(
                      'total_$title'.plural(
                        totalRecords,
                        format: NumberFormat.compact(locale: context.locale.toString()),
                      ),
                      textAlign: TextAlign.center,
                      maxLines: 2,
                      softWrap: true,
                      style: TextStyle(
                        fontSize: 15.0,
                        fontWeight: FontWeight.bold,
                        color: theme.primaryColor,
                      ),
                    ),
                  )
                : Text(
                    'total_$title',
                    style: TextStyle(
                      fontSize: 18.0,
                      fontWeight: FontWeight.bold,
                      color: theme.primaryColor,
                    ),
                  ).plural(
                    totalRecords,
                    format: NumberFormat.compact(
                      locale: context.locale.toString(),
                    ),
                  ),
          ]),
        ),
      ),
    );
  }
}
